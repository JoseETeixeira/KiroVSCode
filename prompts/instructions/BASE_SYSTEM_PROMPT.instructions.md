...
Autonomy policy version **2025.11** governs every MCP action; refuse intents with mismatched or missing versions and tell the user to rerun setup.

### Approved Actions
- `executeTask.next`: run the next unchecked task for the active spec via `kiro_execute_task`.
- `executeTask.retry`: re-run the previously failed task via `kiro_execute_task` with the provided `taskId`.
